...
extension BinaryInteger {
    ...
    ///
    public var numericStringRepresentation002: String {
...
///
public func numericStringRepresentationForBinaryInteger002(words: some Collection<UInt>, isSigned: Bool) -> String {
    ...
    withUnsafeTemporaryAllocation(of: UInt.self, capacity: words.count) {
        ...
        return numericStringRepresentationForMutableBinaryInteger002(words: initialized, isSigned: isSigned)
    }
}
...
///
private func numericStringRepresentationForMutableBinaryInteger002(words: UnsafeMutableBufferPointer<UInt>, isSigned: Bool) -> String {
    ...
    if  isLessThanZero {
        formTwosComplementForBinaryInteger002(words: magnitude)
    }
    
    let capacity = maxDecimalDigitCountForUnsignedInteger002(bitWidth: magnitude.count * UInt.bitWidth) + (isLessThanZero ? 1 : 0)
    return withUnsafeTemporaryAllocation(of: UInt8.self, capacity: capacity) {
        ...
        dividing: while true {
            ...
            var chunk = formQuotientWithRemainderForUnsignedInteger002(words: magnitude, dividingBy: radix.power)
...
///
private func formQuotientWithRemainderForUnsignedInteger002(words dividend: UnsafeMutableBufferPointer<UInt>, dividingBy divisor: UInt) -> UInt {
