...
extension BinaryInteger {
    ...
    ///
    public var numericStringRepresentation001: ArraySlice<UInt8> {
        numericStringRepresentationForBinaryInteger001(words: self.words, isSigned: Self.isSigned)
    }
}
...
words: some Collection<UInt>, isSigned: Bool) -> ArraySlice<UInt8> {
    ...
    withUnsafeTemporaryAllocation(of: UInt.self, capacity: words.count) { copy in
        ...
        return numericStringRepresentationForMutableBinaryInteger001(words: copy, isSigned: isSigned)
    }
}
...
///
private func numericStringRepresentationForMutableBinaryInteger001(
words: UnsafeMutableBufferPointer<UInt>, isSigned: Bool) -> ArraySlice<UInt8> {
    ...
    if  isLessThanZero {
        formTwosComplementForBinaryInteger001(words: words)
    }
    
    let radix: (exponent: UInt, power: UInt) = maxDecimalExponentAndPowerForUnsignedIntegerWord001()
    let capacity =  maxDecimalDigitCountForUnsignedInteger001(bitWidth: words.count * UInt.bitWidth) + (isLessThanZero ? 1 : 0)
    ...
    ascii.withUnsafeMutableBufferPointer { ascii in
        repeat {
            ...
            let remainder = formQuotientWithRemainderForUnsignedInteger001(words: words.prefix(upTo: wordsIndex), dividingBy: radix.power)
            ...
            writeIndex = formTrailingNumericStringRepresentationForUnsignedInteger001(word: remainder, into: ascii.prefix(upTo: asciiIndex))
...
///
private func formTrailingNumericStringRepresentationForUnsignedInteger001(
...
///
private func formQuotientWithRemainderForUnsignedInteger001(
...
///
private func formTwosComplementForBinaryInteger001(words: UnsafeMutableBufferPointer<UInt>) {
    var carry =  true
...
/// ```
///
private func maxDecimalExponentAndPowerForUnsignedIntegerWord001() -> (exponent: UInt, power: UInt) {
...
///
private func maxDecimalDigitCountForUnsignedInteger001(bitWidth: Int) -> Int {
